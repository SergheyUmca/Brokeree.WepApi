...
namespace Brokeree.WepApi.Controllers
{
    public class ResourceV2Controller : ApiController
    {
        ...
        public ResponseModel<GetAllResponseModel> Get()
        {
            ...
            try
            {
                var getCache = ResourceManager.GetValues();
        ...
        public ResponseModel<ResourceObject> Get(string ID, int? startIndex, int? length)
        {
            ...
            try
            {
                ...
                var getResource = ResourceManager.GetValue(vKey);
                if(getResource == null)
                {
        ...
        public ResponseModel Create(string Value, string ID)
        {
            ...
            Guid newId;
            var vKey = Guid.TryParse(ID, out newId) ? Guid.NewGuid() : newId;
            try
            {
                var addResource = ResourceManager.AddPair(vKey, Value);
                if(!addResource)
                {
